...
export const actions: Actions = {
	createAlarm: async ({ request, fetch }) => {
		try {
			...
			const response = await fetch(`http://127.0.0.1:8080/api/v1/wecker/configurations/`, {
				method: 'POST',
				headers: {
	...
	updateAlarmActiveStatus: async ({ request, fetch }) => {
		try {
			...
			const response = await fetch(
				`http://127.0.0.1:8080/api/v1/wecker/configurations/${alarmId}`,
				{
					method: 'PUT',
					headers: {
						'Content-Type': 'application/json',
	...
	deleteAlarm: async ({ request }) => {
		...
		await fetch(`http://127.0.0.1:8080/api/v1/wecker/configurations/${alarmId}`, {
			method: 'DELETE',
			headers: {
				'Content-Type': 'application/json',
				Accept: 'application/json'
